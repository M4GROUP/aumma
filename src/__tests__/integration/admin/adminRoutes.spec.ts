...
import app from "../../../app";
import { mockedAdm, mockedAdminLogin, mockedAdmWithoutEmail, mockedAdmWithoutPassword, mockedNotAdminLogin, mockedWrongAdminLogin } from "../../mocks/admin";
...
describe ("/institutions", () =>{
    ...
    test("POST /admin -  should not be able to create a admin without email",async () => {
        const response = await request(app).post('/admin').send(mockedAdmWithoutEmail)
    ...
    test("POST /admin -  should not be able to create a admin without password",async () => {
        const response = await request(app).post('/admin').send(mockedAdmWithoutPassword)
        expect(response.body).toHaveProperty("message")
        expect(response.status).toBe(400)
    ...
    test("POST /admin/login -  should be able to login",async () => {
        const response = await request(app).post("/admin/login").send(mockedAdminLogin);
    ...
    test("GET /admin/institutions/:id -  Must be able to list the admin`s institutions",async () => {
    ...
    test("GET /admin/institution/:id -  should not be able to list the admin`s institution without authentication",async () => {
    ...
    test("DELETE /admin/institution/:id -  should be able to soft delete the admin`s institution by id",async () => {
        ...
        const adminLoginResponse = await request(app).post("/institutions/login").send(mockedAdminLogin);
        const token = `Bearer ${adminLoginResponse.body.token}`
        const id = adminLoginResponse.body.id_Institution
        
        const response = await request(app).patch(`/admin/institution/${id}`).set("Authorization",token)
        const findUser = await request(app).get(`/admin/${id}`).set("Authorization",token)
    ...
    test("DELETE /admin/institution/:id -  should not be able to soft delete the admin`s institution by id without authentication",async () => {
        const response = await request(app).delete(`/admin/institution/0ac51579-f72b-49e6-b7fc-10a2127af1c4`)
        expect(response.body).toHaveProperty("message")
        expect(response.status).toBe(401) 
